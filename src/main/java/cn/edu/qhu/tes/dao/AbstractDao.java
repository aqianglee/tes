...
public abstract class AbstractDao implements GenericDao {
	...
	@Override
	public <T> List<T> list(Class<T> targetClass, List<? extends Criterion> criterions, Order... orders) {
		return createSelectQuery(targetClass, criterions, emptyJoins(), orders).getResultList();
	}
	...
	private <T> Query createSelectQuery(Class<T> targetClass, List<? extends Criterion> criterions,
	...
	private void contributeJoins(StringBuffer buffer, Map<String, List<Criterion>> joins) {
	...
	private <T> StringBuffer createCountStatement(Class<T> targetClass) {
		StringBuffer buffer = new StringBuffer();
		buffer.append("select count(e) from " + targetClass.getSimpleName() + " e");
		return buffer;
	}

	private <T> StringBuffer createSumStatement(Class<T> targetClass, String fieldName) {
		StringBuffer buffer = new StringBuffer();
		buffer.append("select sum(e." + fieldName + ") from " + targetClass.getSimpleName() + " e");
		return buffer;
	}

	private Map<String, Object> contributeWhereClause(StringBuffer buffer, List<? extends Criterion> criterions) {
	...
	private void contributeJoinWhereClause(Map<String, Object> paramMap, StringBuffer buffer,
		...
		if (!whereClause.isEmpty()) {
			String prefix = buffer.toString().contains("WHERE") ? "AND" : "WHERE";
			buffer.append(" " + prefix + " (" + StringUtils.join(whereClause, ") AND (") + ")");
	...
	private void contributeOrderByClause(StringBuffer buffer, Order... orders) {
		...
		if (!orderBy.isEmpty()) {
			buffer.append(" ORDER BY " + StringUtils.join(orderBy.toArray(), ","));
		}
	}

	@SuppressWarnings("unchecked")
	@Override
	public <T> List<T> list(Class<T> targetClass, List<? extends Criterion> criterions, List<Order> orders, long first,
			long count) {
		Query query = createSelectQuery(targetClass, criterions, emptyJoins(), orders.toArray(new Order[] {}));
	...
	@SuppressWarnings("unchecked")
	@Override
	public <T> List<T> list(Class<T> targetClass, List<? extends Criterion> criterions, List<Order> orders,
			Map<String, List<Criterion>> joins, long first, long count) {
		Query query = createSelectQuery(targetClass, criterions, joins, orders.toArray(new Order[] {}));
		query.setFirstResult(new Long(first).intValue());
		query.setMaxResults(new Long(count).intValue());
		return query.getResultList();
	}
	...
	@SuppressWarnings("unchecked")
	@Override
	public <T> int count(Class<T> targetClass, List<? extends Criterion> criterions) {
		StringBuffer buffer = createCountStatement(targetClass);
		Map<String, Object> paramMap = contributeWhereClause(buffer, criterions);
		List<Number> numbers = (List<Number>) createQuery(buffer, paramMap).getResultList();
		int total = 0;
	...
	@SuppressWarnings("unchecked")
	@Override
	public <T> int count(Class<T> targetClass, List<? extends Criterion> criterions, Map<String, List<Criterion>> joins) {
		StringBuffer buffer = createCountStatement(targetClass);
		contributeJoins(buffer, joins);
		Map<String, Object> paramMap = contributeWhereClause(buffer, criterions);
		contributeJoinWhereClause(paramMap, buffer, joins);
	...
	@SuppressWarnings("unchecked")
	@Override
	public <T> double sum(Class<T> targetClass, String fieldName, List<Criterion> criterions) {
